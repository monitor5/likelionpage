import { useState } from 'react';

const ScoreCard = ({ teamName, members, initialScore }) => {
  const [score, setScore] = useState(initialScore);

  return (
    <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-lg hover:border-blue-500 transition-all group">
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-xl font-bold text-white group-hover:text-blue-400">{teamName}</h3>
        <span className="text-3xl font-black text-yellow-400">{score}</span>
      </div>
      
      <div className="text-slate-400 text-sm mb-6">
        <p className="mb-1">👥 조원</p>
        <div className="flex flex-wrap gap-2">
          {members.map((m, idx) => (
            <span key={idx} className="bg-slate-700 px-2 py-1 rounded text-xs">{m}</span>
          ))}
        </div>
      </div>

      <div className="flex gap-2">
        <button 
          onClick={() => setScore(score + 10)}
          className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded-lg transition-colors cursor-pointer"
        >
          + 10
        </button>
        <button 
          onClick={() => setScore(score > 0 ? score - 10 : 0)}
          className="flex-1 bg-slate-700 hover:bg-red-500 text-white font-bold py-2 rounded-lg transition-colors cursor-pointer"
        >
          - 10
        </button>
      </div>
    </div>
  );
};

export default ScoreCard;